...
bonus2_description = descri_font.render(f"Actual level : {bonus2_level}", True,
                                        cst.WHITE)
bonus2_cost = descri_font.render(f"next level cost : {(bonus2_level ** 2 + 10)}", True, cst.WHITE)
...
bonus3_description = descri_font.render(f"Actual level : {bonus3_level}", True,
                                        cst.WHITE)
bonus3_cost = descri_font.render(f"next level cost : {(bonus3_level ** 3 + 100)}", True, cst.WHITE)
...
def draw_on_screen_in_game():
    ...
    actual_click_damage = descri_font.render(
        f"Actual click value : {((damage + bonus1_level) + ((damage + bonus1_level) * bonus2_level) + ((damage + bonus1_level) * bonus2_level) ** bonus3_level)}",
        True, cst.WHITE)
    ...
    while main:
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and click_area.collidepoint(mouse_x, mouse_y) and event.button == 1:
                score += ((damage + bonus1_level) + ((damage + bonus1_level) * bonus2_level) + ((damage + bonus1_level) * bonus2_level) ** bonus3_level)
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and bonus2_button_rect.collidepoint(mouse_x, mouse_y) and \
                    event.button == 1 and score >= (bonus2_level ** 2 + 10):
                score -= (bonus2_level ** 2 + 10)
                bonus2_level += 1
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and bonus3_button_rect.collidepoint(mouse_x, mouse_y) and \
                    event.button == 1 and score >= (bonus3_level ** 3 + 100):
                score -= (bonus3_level ** 3 + 100)
                bonus3_level += 1
